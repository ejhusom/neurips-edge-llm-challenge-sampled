...
def print_markdown_table(file_data):
    table_header = "| File Name | Dataset - Model | Avg Eval Count |\n| --- | --- | --- |"
    table_rows = []
    for file_name, (eval_counts, _, dataset_model) in file_data.items():
        average_eval_count = sum(eval_counts) / len(eval_counts) if eval_counts else 0
        table_rows.append(f"| {file_name} | {dataset_model} | {average_eval_count:.2f} |")
...
def main():
    ...
    print_markdown_table(file_data)
